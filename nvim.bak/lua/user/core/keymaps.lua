...
-- life savers
keymap.set("n", "<C-a>", "^v$h") -- select current line on text
...
keymap.set("n", "<leader>c", ":BufferClose<CR>") -- close current tab
...
keymap.set("n", "<C-k>", "<C-w>k") --  go to up split

-- plugin keymaps
keymap.set("n", "<leader>sm", ":MaximizerToggle<CR>") -- maximize splits

-- nvim-tree
keymap.set("n", "<leader>e", ":NvimTreeFindFileToggle<CR>") -- maximize splits
...
keymap.set("n", "<leader>tt", "<cmd>FloatermNew<cr>") -- list available help tags
keymap.set(
	"n",
	"<leader>rb",
	":FloatermNew --height=0.9 --autoclose=0 --width=0.9 --wintype=float --name=build --position=center RUSH_BUILD_CACHE_CREDENTIAL=minioadmin:minioadmin rush build -T @aimos/app<CR>"
)
keymap.set(
	"n",
	"<leader>ru",
...
keymap.set(
	"n",
	"<leader>gg",
	":FloatermNew --autoclose=2 --height=0.9 --width=0.9 --wintype=float --name=reRunServer --position=center lazygit<CR>"
)
